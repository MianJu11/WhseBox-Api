...
import com.WhseApi.common.UserStatus;
...
public class UploadController {
    ...
    public Object cosUpload(@RequestParam(value = "file") MultipartFile file, @RequestParam(value = "token", required = false) String  token) throws IOException {
        ...
        if(bi == null){
            return ResultAll.getResultJson(0,"请上传图片文件",null);
        }
        ...
        try {
            ...
            return ResultAll.getResultJson(1,"上传成功",info);

        } catch (IOException e) {
            return ResultAll.getResultJson(0,"上传失败",null);
        }finally {
            // 关闭客户端(关闭后台线程)
            cosclient.shutdown();
        }

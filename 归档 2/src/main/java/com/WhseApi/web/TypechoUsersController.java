...
@ResponseBody
@Slf4j
public class TypechoUsersController extends BaseController {
    ...
    @Autowired
    private TypechoUsersService userService;
...
//                    json.remove("assets");
    ...
    public String userInfo(@RequestParam(value = "key", required = false) String key) {
        try {
            ...
            if (uid < 1) {
                return ResultAll.getResultJson(0, "请传入正确的参数", null);
            }
            TypechoComments comments = new TypechoComments();
            comments.setAuthorId(uid);
            Integer lv = commentsService.total(comments);
    ...
    public String userLogin(@RequestParam(value = "params", required = false) String params) {
        ...
        try {
            ...
            if (rows.size() > 0) {
                ...
                if (!isPass) {
                    return ResultAll.getResultJson(0, "用户密码错误", null);
                }
                //内置验证结束
                Long date = System.currentTimeMillis();
                ...
                jsonToMap.put("inviterId", rows.get(0).getInviterId());
                jsonToMap.put("userLevel", rows.get(0).getUserLevel());
                //判断是否为VIP
            ...
            return ResultAll.getResultJson(rows.size() > 0 ? 1 : 0, rows.size() > 0 ? "登录成功" : "用户名或密码错误", jsonToMap);
        } catch (Exception e) {
            log.error(e.getMessage(),e);
            JSONObject response = new JSONObject();
        ...
        try {
            ...
            if (jsonToMap.get("appLoginType").toString().equals("weixin")) {
                if (jsonToMap.get("type").toString().equals("applets")) {
                    ...
                    if (jsonToMap.get("js_code") == null) {
                        return ResultAll.getResultJson(0, "APP配置异常，js_code参数不存在", null);
                    }
            ...
            if (jsonToMap.get("appLoginType").toString().equals("qq")) {
                if (jsonToMap.get("type").toString().equals("applets")) {
                    ...
                    String res = HttpClient.doGet(requestUrl);
                    if (res == null) {
                        return ResultAll.getResultJson(0, "接口配置异常，QQ官方接口请求失败", null);
                    }
            ...
            if (apiList.size() > 0) {
                ...
                jsonToMap.put("inviterId", user.getInviterId());
                jsonToMap.put("userLevel", user.getUserLevel());
                ...
                return ResultAll.getResultJson(rows > 0 ? 1 : 0, rows > 0 ? "登录成功" : "登陆失败", jsonToMap);

            } else {
                //注册
                if (isInvite.equals(1)) {
                    return ResultAll.getResultJson(0, "当前注册需要邀请码，请采用普通方式注册！", null);
                }
                ...
                jsonToMap.put("vip", 0);
                jsonToMap.put("isvip", 0);
                jsonToMap.put("inviterId", null);
                jsonToMap.put("userLevel", 1);
        ...
        try {
            ...
            if (jsonToMap.get("appLoginType").toString().equals("weixin")) {
                if (jsonToMap.get("type").toString().equals("applets")) {
                    //如果是小程序，走官方接口获取accessToken和openid
                    if (jsonToMap.get("js_code") == null) {
                        return ResultAll.getResultJson(0, "APP配置异常，请检查相关设置", null);
                    }
                    String js_code = jsonToMap.get("js_code").toString();
                    ...
                    String res = HttpClient.doGet(requestUrl);
                    if (res == null) {
                        return ResultAll.getResultJson(0, "接口配置异常，请检查相关设置", null);
                    }
                    ...
                    if (jsonToMap.get("accessToken") == null) {
                        return ResultAll.getResultJson(0, "登录配置异常，请检查相关设置", null);
                    }
                }
            } else {
                if (jsonToMap.get("accessToken") == null) {
        ...
        try {
            if (StringUtils.isNotBlank(params)) {
                ...
                if (isEmail.equals(1)) {
                    ...
                    if (isMail.size() > 0) {
                        return ResultAll.getResultJson(0, "该邮箱已注册", null);
                    }
                }
                toKey.setMail(null);
                ...
                if (isName.size() > 0) {
                    return ResultAll.getResultJson(0, "该用户名已注册", null);
                }
                //验证邮箱验证码
                if (isEmail.equals(1)) {
                    ...
                    if (cur_code == null) {
                        return ResultAll.getResultJson(0, "请先发送验证码", null);
                    }
                    if (!cur_code.equals(code)) {
                        return ResultAll.getResultJson(0, "验证码不正确", null);
                    }
                }
                //验证邀请码
                if (jsonToMap.get("inviteCode") != null) {
                    ...
                    if (invite.size() < 1) {
                        return ResultAll.getResultJson(0, "错误的邀请码", null);
                    } else {
                        TypechoInvitation cur = invite.get(0);
                        cur.setStatus(0);
                        invitationService.update(cur);

                        TypechoUsers inviter = userService.selectByKey(cur.getUid());
                        Integer level = inviter.getUserLevel() == null ? 1 : (inviter.getUserLevel() + 1);
                        jsonToMap.put("inviterId", cur.getUid());
                        jsonToMap.put("userLevel", level);
                    }
                }
            ...
            log.error(e.getMessage(),e);
            return ResultAll.getResultJson(0, "参数错误", null);
        }
    ...
    public String SendCode(@RequestParam(value = "params", required = false) String params, HttpServletRequest request) throws MessagingException {
        try {
            ...
            Integer isEmail = apiconfig.getIsEmail();
            if (isEmail.equals(0)) {
                return ResultAll.getResultJson(0, "邮箱验证已经关闭", null);
            }
            ...
            if (iSsendCode == null) {
                ...
                return ResultAll.getResultJson(0, "你的操作太频繁了", null);
            }
            if (StringUtils.isNotBlank(params)) {
                jsonToMap = JSONObject.parseObject(JSON.parseObject(params).toString());
                ...
                if (isName.size() > 0) {
                    ...
                    return ResultAll.getResultJson(1, "邮件发送成功", null);
                } else {
                    return ResultAll.getResultJson(0, "该用户不存在", null);
                }
            ...
            return ResultAll.getResultJson(0, "不正确的邮箱发信配置", null);
        }
        ...
        try {
            ...
            if (StringUtils.isNotBlank(params)) {
                ...
                if (!baseFull.isEmail(email)) {
                    return ResultAll.getResultJson(0, "请输入正确的邮箱", null);
                }
                //判断邮箱是否寻找
                ...
                List<TypechoUsers> isName = service.selectList(toKey1);
                if (isName.size() > 0) {
                    return ResultAll.getResultJson(0, "该邮箱已被注册", null);
                }
    ...
    public String userFoget(@RequestParam(value = "params", required = false) String params) {
        try {
            ...
            if (StringUtils.isNotBlank(params)) {
                ...
                Integer isEmail = apiconfig.getIsEmail();
                if (isEmail.equals(0)) {
                    return ResultAll.getResultJson(0, "邮箱验证已经关闭，请联系管理员找回密码", null);
                }
                jsonToMap = JSONObject.parseObject(JSON.parseObject(params).toString());
                String code = jsonToMap.get("code").toString();
                ...
                if (redisHelp.getRedis(this.dataprefix + "_" + "sendCode" + name, redisTemplate) != null) {
                    ...
                    return ResultAll.getResultJson(0, "验证码已超时或未发送", null);
                }
                ...
                List<TypechoUsers> isName = service.selectList(toKey1);
                if (isName.size() == 0) {
                    return ResultAll.getResultJson(0, "用户不存在", null);
                }
                ...
                if (jsonToMap.get("code") != null && jsonToMap.get("mail") != null) {
                    ...
                    if (isEmail.equals(1)) {
                        ...
                        if (redisHelp.getRedis(this.dataprefix + "_" + "sendCode" + email, redisTemplate) != null) {
                            ...
                            return ResultAll.getResultJson(0, "验证码不正确或已失效", null);
                        }
                    }
    ...
    public String userWithdraw(@RequestParam(value = "num", required = false) Integer num, @RequestParam(value = "token", required = false) String token) {
        try {
            ...
            TypechoUsers user = service.selectByKey(uid);
            if (user.getPay() == null) {
                return ResultAll.getResultJson(0, "请先设置收款信息", null);
            }
            Long date = System.currentTimeMillis();
            String userTime = String.valueOf(date).substring(0, 10);
            ...
            if (list.size() > 0) {
                return ResultAll.getResultJson(0, "您有正在审核的申请", null);
            }
            userlog.setNum(num);
            userlog.setToid(uid);
    ...
    public String withdrawStatus(@RequestParam(value = "key", required = false) Integer key, @RequestParam(value = "type", required = false) Integer type, @RequestParam(value = "token", required = false) String token) {
        try {
            ...
            if (type.equals(1)) {
                ...
                if (oldAssets < num) {
                    return ResultAll.getResultJson(0, "该用户资产已不足以用于提现！", null);
                }
    ...
    // 获得当天0点时间
    public static int getTimestamp(Integer hour, Integer minute, Integer second) {
    ...
    @ResponseBody
    public String addPoint(@RequestParam(value = "token", required = false) String token) {
        // 获取激励广告增加积分数量
        TypechoApiconfig apiconfig = apiconfigService.selectByKey(1);
        Integer num = apiconfig.getAdAwardPoint();
        Integer adAwardPerDay = apiconfig.getAdAwardPerDay();
        // 查询用户登录状态
        Integer uStatus = UStatus.getStatus(token, this.dataprefix, redisTemplate);
        if (uStatus == 0) {
        ...
        // 判断用户每日新增激励广告次数是否超过限制
        TypechoPaylog paylog = new TypechoPaylog();
        //根据查询条件获取记录
        Integer startTime = getTimestamp(0, 0, 0);
        Integer endTime = getTimestamp(23, 59, 59);
        paylog.setSubject("激励广告");
        paylog.setUid(uid);
        paylog.setStartTime(startTime);
        paylog.setEndTime(endTime);

        int todayAwardTimes = paylogService.selectTotal(paylog);
        if(todayAwardTimes >= adAwardPerDay) {
            return ResultAll.getResultJson(0, "今日奖励已达上限哦！明天再来吧！", null);
        }

        Integer oldAssets = user.getAssets();
        if (num <= 0) {
            return ResultAll.getResultJson(0, "金额不正确", null);
        }
        Integer assets;
        //生成系统对用户资产操作的日志
        Long date = System.currentTimeMillis();
...
//        consumeHelper.cashBack(user.getUid(), standardFee.getPrice());
        consumeHelper.cashBack(uid,num);
        ...
        response.put("code", rows > 0 ? 1 : 0);
        response.put("data", rows);
        response.put("msg", rows > 0 ? "感谢观看，积分已发放" : "观看失败");
        return response.toString();
    }
    ...
    public String getScanStatus(@RequestParam(value = "codeContent", required = false) String codeContent) {
        ...
        if (value == null) {
            return ResultAll.getResultJson(-1, "二维码已过期", null);
        }
        if (value.equals("nodata")) {
            return ResultAll.getResultJson(0, "未扫码", null);
        }
        String token = value;
        Map map = redisHelp.getMapValue(this.dataprefix + "_" + "userInfo" + token, redisTemplate);
        Integer uid = Integer.parseInt(map.get("uid").toString());
    ...
    public String setScan(@RequestParam(value = "codeContent", required = false) String codeContent, @RequestParam(value = "token", required = false) String token) {
        try {
            String value = redisHelp.getRedis(codeContent, redisTemplate);
            if (value == null) {
                return ResultAll.getResultJson(0, "二维码已过期", null);
            }
            redisHelp.setRedis(codeContent, token, 90, redisTemplate);
            return ResultAll.getResultJson(1, "操作成功！", null);
        } catch (Exception e) {
            return ResultAll.getResultJson(0, "请求异常", null);
        }
    ...
    public String madeInvitation(@RequestParam(value = "num", required = false) Integer num, @RequestParam(value = "token", required = false) String token) {
        try {
            ...
            Map map = redisHelp.getMapValue(this.dataprefix + "_" + "userInfo" + token, redisTemplate);
            Integer userLevel = map.get("userLevel")==null?0:Integer.parseInt(map.get("userLevel").toString());
            if(userLevel==3){
                return ResultAll.getResultJson(0, "不可生成邀请码！", null);
            }
            Integer uid = Integer.parseInt(map.get("uid").toString());
            if (num > 100) {
                num = 100;
    ...
    public String invitationList(@RequestParam(value = "searchParams", required = false) String searchParams,
        ...
        query.setUid(Integer.parseInt(map.get("uid").toString()));
     ...
     * 购买刷新次数
     */
    @PostMapping(value = "/buyRenovate")
    public String buyRenovate(@RequestHeader("token") String token,
                              @RequestBody @Validated({BuyRenovateGroup.class}) ContentQuery buy,
        ...
        TypechoUsers userInfo = userService.selectByKey(user.getInteger("uid"));
        service.buyRenovate(userInfo, buy);
    ...
    public String read(@RequestHeader("token") String token,
                       @RequestBody @Validated({ReadGroup.class}) ContentQuery read,
                       BindingResult result) throws ValidationException {
        super.validate(result);
        JSONObject user = UStatus.getUser(token, dataprefix, redisTemplate);
        if (user == null) {
        ...
        TypechoUsers userInfo = userService.selectByKey(user.getInteger("uid"));
        int resultCode = service.read(userInfo, read);

        return ResultAll.getResultJson(resultCode, null, null);
    }

    /**
     * 购买刷新次数
     */
    @GetMapping(value = "/getRenovate")
    public String getRenovate(@RequestHeader("token") String token) {
        JSONObject user = UStatus.getUser(token, dataprefix, redisTemplate);
        if (user == null) {
            return ResultAll.errorToken();
        }
        TypechoUsers userInfo = userService.selectByKey(user.getInteger("uid"));
        return ResultAll.getResultJson(1, null, userInfo.getRenovate());
    }
    ...
    public String superior(@RequestHeader("token") String token){
        JSONObject user = UStatus.getUser(token, dataprefix, redisTemplate);
        if (user == null) {
            return ResultAll.errorToken();
        }
        UserVO superior = userService.getSuperior(user.getInteger("uid"));

        TypechoApiconfig apiconfig = apiconfigService.selectByKey(1);
        if(superior.getAvatar()==null){
            if (superior.getMail() != null) {
                superior.setAvatar(baseFull.getAvatar(apiconfig.getWebinfoAvatar(), superior.getMail()));
            } else {
                superior.setAvatar(apiconfig.getWebinfoAvatar() + "null");
            }
        }
        return ResultAll.getResultJson(1, null, superior);
    }
    ...
    public String subordinates(@RequestHeader("token") String token) {
        JSONObject user = UStatus.getUser(token, dataprefix, redisTemplate);
        if (user == null) {
            return ResultAll.errorToken();
        }
        List<UserVO> list = userService.getSubordinates(user.getInteger("uid"));
        TypechoApiconfig apiconfig = apiconfigService.selectByKey(1);
        for (UserVO userVO : list) {
            if(userVO.getAvatar()==null){
                if (userVO.getMail() != null) {
                    userVO.setAvatar(baseFull.getAvatar(apiconfig.getWebinfoAvatar(), userVO.getMail()));
                } else {
                    userVO.setAvatar(apiconfig.getWebinfoAvatar() + "null");
                }
            }
        }
        return ResultAll.getListResult(1, null, list);
    }
